...
def call_fortran_tanh(data,_in_node,_out_node,_rc_node,_samp_num,_samp_step,_all_step):
...
#        np.savetxt('./data_out/np_savetxt_s.txt', y_tr,fmt='%.3e')
        ...
        Wout = Wout.T.copy().astype('float64')
...
call_fortran_tanh(data,ndim,one_hot_l.shape[1],rc_node,data.shape[0],nstep,
                        int(data.shape[0]*nstep))

#####2次元プロット
plt.plot(DATASET[:,0],"-" , label="ori")
plt.legend(loc=2)
plt.show()