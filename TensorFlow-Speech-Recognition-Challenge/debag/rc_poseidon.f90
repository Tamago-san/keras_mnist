...
subroutine rc_poseidon(in_node00,out_node00,rc_node00,&
                    traning_step00,rc_step00,&
                    u_tr,s_tr,u_rc,s_rc_data,s_rc,w_out0)
  use cylinder
    implicit none
    integer(4), intent(inout) :: in_node00,out_node00,rc_node00
    integer(4), intent(inout) :: traning_step00,rc_step00
...
!                isample=istep/samp_step00 +1
                ...
                if (mod(istep,100).eq.0) call output(Vx,Vy,P)
!                do i=1,in_node
!                    U_data(1,i) = u_tr(istep,i)
!                enddo
                U_data(1,1) =  u_tr(istep,1)
                U_data(1,2) = -u_tr(istep,1)
                U_data(1,3) = -u_tr(istep,1)
                U_data(1,4) =  u_tr(istep,1)
                do i=1,out_node
...
!                        DATA(Future3,1),DATA(Future3,2),DATA(Future3,3),istep)
                ...
                U_data(1,2) = -u_rc(istep,1)
                U_data(1,3) = -u_rc(istep,1)
                U_data(1,4) =  u_rc(istep,1)
                
                do i=1,out_node
                    S_data(1,i) = s_rc_data(istep,i)
                    !if(s_tmp(1,i)<1.d-10) s_tmp(1,i) =0.d0
                enddo
!                call DATA_standard(2)
                call march(Vx,Vy,P,1)
                call create_r_matrix(Vx,Vy,r_tmp,istep)
                call RC_OWN(Vx,Vy,r_tmp,s_rc(istep,1:out_node),istep)
            ...
            enddo
            !close(45)
            open(60,file='./data_out/a.dat')
            rc_err =0.d0
            do istep=1,rc_step
                rc_err= rc_err +(s_rc_data(istep,1)-s_rc(istep,1))**2
                write(60,*) s_rc(istep,1),s_rc_data(istep,1)
            enddo
            close(60)
            !call out_result
            write(*,*) "=========================================="
            write(*,*) "     RC ERR >>>>>",rc_err/dble(rc_step)
            write(*,*) "=========================================="
