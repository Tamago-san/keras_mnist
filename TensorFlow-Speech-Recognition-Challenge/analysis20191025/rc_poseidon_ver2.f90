!-----------------------------------------------------------------------------
!gfortran -shared -o rc_poseidon.so rc_poseidon_ver2.f90 -llapack -fPIC
!-----------------------------------------------------------------------------
module cylinder
  ...
  integer, parameter :: OUT_NODE = 3
  integer, parameter :: IN_NODE = 2
  ...
  real(kind=8) :: TYO =1.d0 !信号の大きさ（１なら大きさ１）
  real(kind=8) :: TYK =1.d0 !5なら信号の大きさ1に（5をデフォルトとする）（-0.5～+0.5）
  ...
  integer(kind=4) :: istep,idtldtN,iRe_int,isample
  ...
  real(kind=8) :: P_tmp(NXmin:NXmax,NYmin:NYmax)
...
!-------------------------------------------------------
  ...
  !real(kind=8) :: U_data (1,IN_NODE*2)
  real(kind=8) :: U_data (1,4)
...
!-----------------------------------------------------------------------------
  ...
  real(kind=8) :: Ry(NXmin:NXmax,NYmin:NYmax)
  real(kind=8) :: R
  real(kind=8) :: sini,cosi,itheta,ic_dble
  integer :: step,O_O,inode
  integer :: ib,ic_int
    ...
    do iY=iY1(iX),iY2(iX)
        ...
        Y=dX*real(iY,8)
        itheta=atan2(Y,X)
        if(itheta<0) itheta=itheta +2*PI
        ib=int( itheta *6.d0/PI)+1!do ib =1,12
        
        ic_dble = dble((ib+1)/3)
        ic_int  = (ib+1)/3
        if(mod(ib+1,3) == 0 ) then
            u_tmp = U_data(1,ic_int)*3.d0*cos(6.d0*(itheta - 0.25d0*(2.d0*ic_dble-1.d0)*PI ) )
            u_tmp = ( u_tmp/TYK )*TYO + TYU
            !write(*,*) ''
            !write(*,*) ic_int,ic_dble,ib,itheta/(2*pi) *360.d0
            !write(*,*) cos(6.d0*(itheta - 0.25d0*(2.d0*ic_dble-1.d0)*PI ) )
            !write(*,*) u_tmp,U_data(1,ic_int)
        else
            u_tmp = 0.d0
            !write(*,*) ''
            !write(*,*) itheta/(2*pi) *360.d0
            !write(*,*) u_tmp
        endif
        Rx(iX,iY)=Rx(iX,iY)+( X * u_tmp - Vx(iX,iY))/dt
        Ry(iX,iY)=Ry(iX,iY)+( Y * u_tmp - Vy(iX,iY))/dt
...
!-----------------------------------------------------------------------------
  ...
  C1=1.d0/(2.d0*dX)
  open(20,file='./data_out/data_out/velocity.'//cistep)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine create_r_matrix(Vx,Vy,r_tmp,time)
        ...
        enddo
        enddo
        r_tmp(RC_NODE)=dble(in_True)*(( U_data(1,1)/TYK )*TYO + TYU)
        r_tmp(RC_NODE-1)=dble(in_True)*(( U_data(1,2)/TYK )*TYO + TYU)
        r_tmp(RC_NODE-2)=1.d0
!        write(*,*) inode,inode2
!        do i= 1,tate_y
!            do j = 1,yoko_x
...
!        err_tmp = 0.d0
    ...
    subroutine CAL_RC_ACC(acc,err,acc_array,s_rc,s_rc_data)
        real(8) acc,err
        real(8) s_rc(rc_num,out_node)
        real(8) s_rc_data(rc_num,out_node)
        integer  result_rc(1),result_data(1)
        real(8)  acc_array(out_node,out_node)  !出力次元数、列サイズはトレーニング時間
...
!                err = err + (s_rc(isample,inode) - s_rc_data(isample,inode))**2
                err = err + (s_rc(isample,inode)/dble(samp_num) - s_rc_data(isample,inode))**2
            enddo
        enddo
...
subroutine rc_poseidon(in_node00,out_node00,rc_node00,&
                    ...
                    u_tr,s_tr,u_rc,s_rc_data,w_out0,acc_array)
  use cylinder
    implicit none
    ...
    real(8) s_rc(rc_step00,out_node)  !出力次元数、列サイズはトレーニング時間
    ...
    !-----------------------------------------------------------------------------------------------------------------
        ...
        write(*,*) "=========================================="
            do istep=1,traning_step00
                ...
                if (mod(istep,1000).eq.0) call output(Vx,Vy,P)
!                do i=1,in_node
!                    U_data(1,i) = u_tr(istep,i)
!                enddo
                U_data(1,1) =  u_tr(istep,1)
                U_data(1,2) = -u_tr(istep,2)
                U_data(1,3) = -u_tr(istep,1)
                U_data(1,4) =  u_tr(istep,2)
                do i=1,out_node
...
!                        DATA(Future3,1),DATA(Future3,2),DATA(Future3,3),istep)
                !do i=1,in_node
                U_data(1,1) =  u_rc(istep,1)
                U_data(1,2) = -u_rc(istep,2)
                U_data(1,3) = -u_rc(istep,1)
                U_data(1,4) =  u_rc(istep,2)
...
!                call DATA_standard(2)
            ...
            enddo
            call CAL_RC_ACC(rc_acc,rc_err,acc_array,s_rc,s_rc_data)
            write(*,*) "=========================================="
            write(*,*) "     RC ACC >>>>>",rc_acc/dble(rc_num) *100.d0,'%'
            write(*,*) "=========================================="
            write(*,*) "=========================================="
            write(*,*) "     RC MSE >>>>>",rc_err/dble(out_node*rc_num),'mse'
            write(*,*) "=========================================="
